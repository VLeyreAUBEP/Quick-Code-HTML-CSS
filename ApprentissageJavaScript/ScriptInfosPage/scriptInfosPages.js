...
javascript:(function() {
  ...
  function plateformInf(hrefPage, plateform) {
    const indexPlat = hrefPage.indexOf(plateform);
    const boolPlat = (indexPlat != -1);
    return boolPlat;
  }

  /* Traitement des infos en fct de la plateforme choisie */
  const boolRoadwork = plateformInf(hrefPage, "roadwork");
  const boolDyna = plateformInf(hrefPage, "dyna");
  const boolAurore = plateformInf(hrefPage, "aurore");
  ...
  if (boolRoadwork || boolAurore) {
    const eleCo = document.querySelectorAll(".header__title span");
    const eleCoPrin = document.querySelector(".liens_menu_selected");
    cockpit = (
      eleCo.length != 0
        ? eleCo.length <= 4
          ? eleCo[0].textContent.trim().split(/\s+/).slice(0, 3).join(' ') == "Tableau de bord"
            ? eleCoPrin?.textContent ?? "Cockpit non trouvé"
            : eleCo[0].textContent + (eleCoPrin ? " ➔ " + eleCoPrin.textContent : "")
          : eleCo[0].textContent + "  ⮞  " + eleCo[1].textContent
        : "Cockpit non trouvé"
    );
  } else if (boolDyna) {
    const eleCo = document.querySelectorAll(".head_path_info");
    const eleCoPrin = document.querySelector(".liens_menu_selected");
    let path = "";

    for (let i = 0; i < eleCo.length; i++) {
      const innerTd = eleCo[i].querySelector("td"); // premier <td> descendant
      if (innerTd && innerTd.textContent.trim() !== "") {
        path += (i == eleCo.length - 1
  ...
  ul.appendChild(createLi("Gravité du bug :", `
    <label style="margin-right:10px;"><input type="radio" name="gravite" value="bloquant" id="bloquant" checked> Bloquant</label>
    <label style="margin-right:10px;"><input type="radio" name="gravite" value="critique" id="critique"> Critique</label>
    <label><input type="radio" name="gravite" value="pasUrgent" id="pasUrgent"> Pas urgent</label>
  `));
